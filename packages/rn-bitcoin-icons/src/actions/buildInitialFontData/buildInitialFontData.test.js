...
describe('buildInitialFontData', () => {
  ...
  it('should throw if there is no previousIconCodes', async () => {
    await expect(buildInitialFontData()).rejects.toEqual(
      new Error('previousIconCodes not found')
    );
  });

  it('should throw if there is no cleanedSvgOutput', async () => {
    await expect(
      buildInitialFontData({ previousIconCodes: {} })
    ).rejects.toEqual(new Error('cleanedSvgOutput not found'));
  });
  ...
  it('should create a metadataProvider calling getMetadataProvider', () => {
    ...
    return buildInitialFontData(data).then(() =>
      expect(getMetadataProviderSpy).toHaveBeenCalledWith(data)
    );
  });

  it('should buildInitialFontData', () => {
    jest.spyOn(metadataBuilder, 'getMetadataProvider').mockReturnValue(jest.fn);
    ...
    return buildInitialFontData(data).then(() =>
      expect(webfont).toHaveBeenCalledWith(expectedConfig)
    );
  });
});
