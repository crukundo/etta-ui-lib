...
import { pipe, prop, map, fromPairs, mergeDeepRight, keys, join } from 'ramda';

const addColorToPaths = xml =>
...
const buildTxt = (drawables, data) => {
  ...
  const iconNames = pipe(keys, join('\n'))(drawables);
...
const convertToDrawable = data => item =>
  svg2vectordrawable(item.contents).then(handleDrawable(item, data));
...
const buildPromises = data =>
  pipe(prop('glyphsData'), map(convertToDrawable(data)))(data);

export const buildDrawable = data => {
  const drawables = buildPromises(data);

  return Promise.all(drawables).then(buildOutputs(data));
};
