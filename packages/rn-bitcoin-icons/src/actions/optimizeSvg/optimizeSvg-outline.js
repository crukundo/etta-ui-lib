...
import { svgoConfig } from './svgoConfig-outline';

const writeCleanedSvg =
  iconsData =>
  ({ data, path: filePath }) => {
    if (!fs.existsSync(iconsData.globalConfig.cleanedSvgOutput)) {
      fs.mkdirSync(iconsData.globalConfig.cleanedSvgOutput, {
        recursive: true,
      });
    }

    const outputPath = path.resolve(
      iconsData.globalConfig.cleanedSvgOutput,
...
export const optimizeSvgOutline = iconsData => {
  ...
  const promises = pipe(R.path(dataPath), map(cleanSvg(svgo)));

  // eslint-disable-next-line no-undef
  return Promise.all(promises(iconsData)).then(handleCleanedSvgs(iconsData));
};

export default optimizeSvgOutline;
